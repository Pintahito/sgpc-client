...
const ObrasDetails = ({
  ...
  //implementaciones prueba

  // Recuperar el estado del cronograma desde localStorage al cargar
  useEffect(() => {
    const storedSchedule = localStorage.getItem(`schedule_${obra.id}`);
    const storedViewing = localStorage.getItem(`viewing_${obra.id}`);

    if (storedSchedule) {
      setScheduleData(JSON.parse(storedSchedule));
    }
    if (storedViewing === "true") {
      setViewingSchedule(true);
    }
  }, [obra.id]);

  // Guardar el estado del cronograma en localStorage al cambiar
  useEffect(() => {
    if (scheduleData) {
      localStorage.setItem(`schedule_${obra.id}`, JSON.stringify(scheduleData));
    }
    localStorage.setItem(`viewing_${obra.id}`, viewingSchedule.toString());
  }, [scheduleData, viewingSchedule, obra.id]);

  const handleViewSchedule = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${apiUrl}/api/v1/schedules?id=${obra.idSchedule}`);
      ...
      alert("Hubo un error al cargar el cronograma.");
    } finally {
      setLoading(false);
    }
  };
...
//aqui acaban las implementacione nuevas
